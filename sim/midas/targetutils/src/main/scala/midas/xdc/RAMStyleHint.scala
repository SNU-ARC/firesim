...
object RAMStyleHint {
  ...
  def apply(mem: chisel3.MemBase[_], style: RAMStyle): Unit = {
    val currentModule = chisel3.Module.currentModule.get
    def memToTarget = {
      require(currentModule.name == mem.parentModName,
      s"""|RAMStyleHints must be module local and not on instance paths.)
          |  Module containing memory: ${mem.parentModName}
          |  Module in which annotator was invoked: ${currentModule.name}
          |""".stripMargin)

      mem.toTarget
    }
    annotate(style, memToTarget)
