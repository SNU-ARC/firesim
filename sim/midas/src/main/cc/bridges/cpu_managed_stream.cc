 ...
 * @param threshold_bytes Minimum number of bytes to enqueue. If fewer bytes
 *        would be enqueued, this method enqueues none and returns 0.
 * @return size_t
 */
size_t StreamFromCPU::push(void* src, size_t num_bytes, size_t threshold_bytes) {
  assert(num_bytes >= threshold_bytes);
  ...
  int num_beats = num_bytes / DMA_BEAT_BYTES;
  int threshold_beats = threshold_bytes / DMA_BEAT_BYTES;

  assert(threshold_beats <= this->fpga_buffer_size());
  int space_available = this->fpga_buffer_size() - this->mmio_read(this->count_addr());
  std::cout << "Count Address: " << this->count_addr() <<" Space Available: " << space_available << " Threshold: " << threshold_beats << std::endl;
  ...
  std::cout << "Push beats: " << push_beats << std::endl;
 ...
 * @param threshold_bytes  Minimum number of bytes to dequeue. If fewer bytes would
 *        be dequeued, dequeue none and return 0.
 * @return size_t Number of bytes successfully dequeued
 */
size_t StreamToCPU::pull(void* dest, size_t num_bytes, size_t threshold_bytes) {
  assert(num_bytes >= threshold_bytes);
  ...
  size_t num_beats = num_bytes / DMA_BEAT_BYTES;
  size_t threshold_beats = threshold_bytes / DMA_BEAT_BYTES;

  assert(threshold_beats <= this->fpga_buffer_size());
  size_t count = this->mmio_read(this->count_addr());

  if ((count == 0) || (count < threshold_beats)) {
    return 0;
  }

  size_t pull_beats = std::min(count, num_beats);
  size_t pull_bytes = pull_beats * DMA_BEAT_BYTES;
  size_t bytes_read = this->pcis_read(this->dma_addr(), (char*) dest, pull_bytes);
