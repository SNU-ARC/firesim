...
class CPUManagedStream {
  public:
    CPUManagedStream(
      ...
      std::function<data_t(size_t)> mmio_read_func) :
...
class StreamFromCPU : public CPUManagedStream {
  public:
    ...
    size_t push(void* src, size_t num_bytes, size_t threshold_bytes);
