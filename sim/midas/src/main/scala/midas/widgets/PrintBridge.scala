...
class PrintBridgeModule(resetPortName: String, printPorts: Seq[(firrtl.ir.Port, String)])(implicit p: Parameters)
  ...
  lazy val module = new BridgeModuleImp(this) {
    ...
    val packingRatio = if (pow2Bits < BridgeStreamConstants.streamWidth) Some(BridgeStreamConstants.streamWidth/pow2Bits) else None
    ...
    if (pow2Bits == BridgeStreamConstants.streamWidth) {
      ...
      val mwFifoDepth = math.max(1, 1 * pow2Bits/BridgeStreamConstants.streamWidth)
      val widthAdapter = Module(new junctions.MultiWidthFifo(pow2Bits, BridgeStreamConstants.streamWidth, mwFifoDepth))
